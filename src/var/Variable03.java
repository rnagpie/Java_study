...
public class Variable03 {
	...
	public static void main(String[] args) {
		...
		//int i3 = 3.33; // int 정수형 <-실수 3.33 저장불가 
		
		double d3 = i1; // double <-int 자동타입 변환 큰타입 = 작은 타입
	
		int i3 =(int)d2; //작은타입 = (작은타입)큰타입 강제 타입 변환
...
//		int result2 = i6 + d6; // int + double
		...
		int rst = (int)dv1 + (int)dv2;
				//  5           6
   		System.out.println(rst);      //11  int 라서 소수점 제외하고 계산 
		
		rst = (int)(dv1 + dv2 );  // 12.1 int 라서 소수점 0.1 표기 없음
		//           12 
		...
		// 소수점이 정확하게 겨로가로 나오게 하기위해서 
		// 연산을 double 과 int 으 연산으로 바꾼다
		...
		z = 10 / 3.0 ; // 소수점을 넣으면 double로 인식한다.
	}
